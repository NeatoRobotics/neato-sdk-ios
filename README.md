...
During the registration of your Neato App on the Neato Developer Portal you have defined a `Redirect URI`. This is the URL where we redirect a user that completes a login with your Neato App Client ID. Your iOS App must be able to handle this Redirect URI using to a dedicated `Schema URL`. To register it you can click on your app target on Xcode, select the `info` tab a under `URL types` add a new URL that must be equal to your Redirect URI. We suggest to use something specific and unique like `MyCompanyNeatoCommander://` to avoid interferences with other applications that might have registered the same URL.
...
You can choose when to present a login page to your users. The easiest way to go is to call the `openLoginInBrowser` function from an instance of `NeatoAuthentication` (This class is implemented as singleton, so you can easily access its shared instance):
...
Before sending a command to a robot you should verify the robot supports that command and, depending on the service supported, you decide which function to call and whith which parameters.
